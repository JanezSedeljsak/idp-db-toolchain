...
def metrics_path() -> Path:
    return data_dir() / ".backupper-metrics.json"


def slow_queries_path() -> Path:
    return data_dir() / ".backupper-slow-queries.jsonl"


def collect_db_metrics(sess: Session) -> dict[str, Any]:
...
def collect_slow_queries(sess: Session, *, min_ms: int) -> list[dict[str, Any]]:
...
def persist_metrics(cfg: Config) -> dict[str, Any]:
    ...
    with session(cfg.database_url) as sess:
        metrics = collect_db_metrics(sess)
...
def persist_slow_queries(cfg: Config) -> list[dict[str, Any]]:
    with session(cfg.database_url) as sess:
        slow = collect_slow_queries(sess, min_ms=cfg.slow_query_ms)
    if slow:
        with slow_queries_path().open("a") as fh:
            for row in slow:
    ...
    if metrics.get("last_backup_at"):
        ...
        lines.extend(
            [
                ...
                f"backupper_last_backup_timestamp_seconds {ts}",
            ]
        )
    return "\n".join(lines) + "\n"
